...
### Deploy it to a URL you can share

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/rameshs-rama/cnc)

`render.yaml` describes the whole thing. Both signing secrets are generated by
Render, nothing has to be typed in, and the pilot factory and benchmark part
are seeded on first boot. When it finishes you have a live
`https://cnc-platform-api.onrender.com/docs` to explore and share, and the
workspaces at `https://cnc-platform-web.onrender.com`.

This is an evaluation profile. Render's free instance type has no persistent
disk, so the database and the object store reset whenever the service restarts
or wakes from idle — released packages and audit history do not survive that.
`render.yaml` carries the Postgres and disk configuration to switch to, and
[docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) has the pre-production checklist.

### The workspaces, hosted by GitHub
...
site can be pointed at any deployment without a rebuild.
